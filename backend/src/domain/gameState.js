 ...
 * @property {{playerId: string, reason: ('THIRD_DOUBLE'|'GO_TO_JAIL_TILE'|'EVENT_CARD'), viaPosition: (number|null)}|null} lastJailEvent - Display-only (2026-09-04): WHO was just sent to jail and WHY. Null until the first jailing of the match, then simply overwritten by the next one — a "last known value", never a pending/consumed flag (same standing as lastDrawnEventCardId).
 *
 *   Exists because the reason is genuinely unreconstructable client-side, and its absence read to real players as a rules BUG. Measured over 400 fuzzed CLASSIC matches: only 223 of 2543 jailings (8.8%) came from three consecutive doubles — 1935 came from landing on the go_to_jail tile and 385 from an event card. In every one of those cases the client saw a token vanish to the jail corner with no explanation at all: sendToJail() rewrites currentPosition straight to the jail tile, so the go_to_jail tile is never a position the client is told about, and the walk animation can never match it (player at P rolling T to land on the tile satisfies P+T = tilePos, while the client sees a delta of jailPos-P — equal only if jailPos = tilePos, which is never). The streak that caused a THIRD_DOUBLE is likewise reset to 0 in the same transition, so it is gone before any broadcast.
 *
 *   `viaPosition` is the board position that CAUSED it — the go_to_jail tile for GO_TO_JAIL_TILE, null for the other two reasons (a 3rd double and a card both jail you from wherever you stand). The board walks the token through it before dropping the player into jail, so the cause is visible rather than inferred.
 *
 *   No game logic reads this back; it is purely what the table is shown.
 * @property {number} lastJailEventSeq - companion counter to lastJailEvent, exactly the role lastRollSeq/lastTrapHitSeq/lastDrawnEventCardSeq play for theirs: `stateVersion` bumps on every action anyone takes, so a client keying a one-shot announcement off it would replay that announcement on every later unrelated action (finding #37's bug). Incremented by exactly 1 only where lastJailEvent is genuinely set. Starts at 0, never reset — **reading it needs `?? 0` at any arithmetic site**, since a match already in progress is restored straight from snapshot JSONB and bypasses createGameState()'s default.
...
export function createGameState(fields) {
  ...
  return {
    ...
    lastJailEvent: fields.lastJailEvent ?? null,
    lastJailEventSeq: fields.lastJailEventSeq ?? 0,
