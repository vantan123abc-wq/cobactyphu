...
// The rider follows the OWNER, never the payer (2026-09-04). Worth pinning
// separately because INFRA is the one modifier here with no tile-type guard —
// it applies to every rent its owner collects — so an implementation that
// resolved the tier against the wrong player would still look correct in every
// test above, where owner and utility-holder happen to be the same person.
test('ASYMMETRIC: the payer holding both utilities does not discount or inflate what they owe', () => {
  const board = [tile(1, 'red'), tile(3, 'red'), utility(6), utility(28)];
  // p2 owns the two CONTROL tiles being landed on; p1, the PAYER, owns both
  // utilities. p2 therefore has CONTROL tier 1 (+50%) and no INFRA at all.
  const properties = [prop(1, 'p2'), prop(3, 'p2'), prop(6, 'p1'), prop(28, 'p1')];
  const gameState = { ruleset: 'ASYMMETRIC', players: [{ id: 'p1' }, { id: 'p2' }], properties };
  const rent = calculateFinalRent(gameState, 'p1', 'p2', board[0], properties[0], [], undefined, 7, board);
  assert.strictEqual(rent, 150, "the payer's own INFRA tier must not touch rent they are paying");
});
