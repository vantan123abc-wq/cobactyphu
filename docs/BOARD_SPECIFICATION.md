...
**The host chooses the opening bid — [NEW 2026-09-03].** `FORCE_AUCTION` takes an optional `basePrice`. Omitted, the auction opens at the tile's printed price (V1 behaviour, unchanged). Supplied, it must be an integer in `[ceil(price × 0.5), price]`: the host may **discount** the opening to attract a bid, but never open above the printed price. The floor is `MIN_AUCTION_OPEN_RATIO = 0.5` — V0's own original fixed opening bid, reused rather than invented — and exists because an unbounded floor turns "open at $1" into a collusion tool at 3+ players. The ceiling exists because opening above the printed price cannot attract a bid the printed price would not; it only guarantees a FAILED auction that still burns the fee, which skipping already does for free. **The fee is charged on the printed price, not on the chosen opening**, so discounting the opening never discounts the host's own cost — otherwise the auction most generous to the buyer would also be the cheapest one to run. Motivated by the heads-up measurement above: at a 2-player table 88% of forced auctions failed because the sole eligible bidder could not afford `printed + $10`, and the opening bid was the only lever that moved that number.

**The host cannot bid in their own auction — [REVISED 2026-09-02].** `handleForceAuction` excludes the initiator from `eligibleBidders`, so they never enter `activeBidders` and no `PLACE_BID` from them is accepted. The commission *is* their stake in the outcome; letting them also bid would have made forcing an auction a way to buy the lot while being paid 20% of your own price. Note the consequence at a two-player table: forcing an auction leaves exactly **one** eligible bidder, so the lot goes to the opponent at base price or not at all. Superseded the V1 rule where the initiator bid like anyone else — `GAME_STATE_MACHINE.md` §Flash Auction and the auction test helpers (named for how many players can actually bid, which is one fewer than sit at the table) were updated with it.
